...
const ProfileForm = ({ user }) => {
    ...
    const saveHandler = async () => {
        ...
        if(response.status === 200) {
            dispatch(modalActions.setModal(true, {header: 'profile updated', body: 'Your profile was updated successfully', error: false}))
            dispatch(userActions.setUser(response.user))
        } else {
            dispatch(modalActions.setModal(true, {header: 'update failed', body: 'Your profile was not updated', error: true}))
        }
    }
    ...
    return (
        <div className='form'>
            <div className='form__container'>
                ...
                <button className='btn__active mauto' onClick={ saveHandler }>Save</button>
            </div>
        </div>
    )
}
