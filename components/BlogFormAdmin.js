    ...
    const createBlogHandler = async () => {
        ...
        if(response.status === 201) {
            dispatch(modalActions.setModal(true, {
                header: 'blog created', 
                body: `Your blog "${response.blog.blog_title}" was created successfully`, error: false
            }))
            setTitle('')
            setIntroduction('')
            setBody('')
            ...
            dispatch(modalActions.setModal(true, { 
                header: 'creation of blog failed', body: 'Your blog could not be created', error: true
            }))
        }
    }

    const updateBlogHandler = async () => {
        const response = await request(`/api/blog/blog_code/${blog.blog_code}`, 'PUT', {
            title: title,
            introduction: introduction,
            body: body,
            conclusion: conclusion,
        ...
        if(response.status === 200) {
            ...
            dispatch(modalActions.setModal(true, { 
                header: 'update of blog failed', body: 'Your blog could not be updated', error: true
            }))
        } 
    }
        ...
        <div className='form'>
            <div className='form__container'>
...

                {
                    blog.blog_code ? (
                        <div className='flex'>
                            <button className='btn__active mauto' onClick={ previewHandler }>Preview</button>
                            <button className='btn__active mauto' onClick={ updateBlogHandler }>Update</button>
                        </div>
                    ) : (
                        <button className='btn__active mauto' onClick={ createBlogHandler }>Create</button>
                    )
                }
            </div>
        </div>
