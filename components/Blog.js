...
const BlogContent = ({ blog, author }) => {
    ...
    if(blog.blog_body) {
        blog_introduction = blog.blog_introduction.split('\n').map((line, index) => (
                <p key={ 'intro_' + index }>{ line }</p>
        ))

        blog_body = blog.blog_body.split('\n').map((line, index) => (
            <p key={ 'body_' + index }>{ line }</p>
        ))

        blog_conclusion = blog.blog_conclusion.split('\n').map((line, index) => (
            <p key={ 'conclusion_' + index }>{ line }</p>
        ))
    }

    const blog_creation_date = moment(blog.blog_creation_date).format("MMM Do YY")
        ...
        <Fragment>
            <div className='blog__title-container'>
                ...
                <span className='blog__author'>
                    <span className='blog__author-date'>
                        { blog_creation_date }   
                    </span>
